...
class BotEngine {
  constructor() {
    ...
    // Manages 6-10 chatters (mixed female & male) actively speaking on main
  ...
  // Continuous channel chatter driven by active 6-10 chatters on configured channel
  scheduleChatter() {
    ...
    const timer = setTimeout(() => {
      ...
      if (b) {
        ...
        if (Math.random() < config.dynamics.doubleMessageChance) {
          ...
          const line = this.humanize(pick(pool), this.userNick, b.nick);
  ...
  triggerBurst(channel, triggerNick) {
    ...
    for (let i = 0; i < burstCount; i++) {
      ...
      accumulatedDelay += rand(1500, 2800);
    }
  }

  // Frequent join & part scheduling (7-18 seconds) on configured channel WITHOUT parting messages
  scheduleJoinLeave() {
  ...
  // Inter-bot dialogues on configured channel
  scheduleInterBotChat() {
    ...
    const timer = setTimeout(() => {
      ...
      if (onlineBots.length >= 2) {
        const dialogue = pick(dialogs.interBotDialogues);
        const starter = this.state[dialogue.starter];
        const responder = this.state[dialogue.responder];

        if (starter && starter.online && responder && responder.online) {
          this.sayWithTyping(chan, starter.nick, dialogue.line);
          setTimeout(() => {
            if (responder.online) {
              this.sayWithTyping(chan, responder.nick, dialogue.response);
            }
          }, rand(2200, 4500));
        } else {
          // Fallback: pick one female and one male from active speakers
          ...
          if (a && b) {
            ...
            const lineA = pick(dialogs.smallTalk[genderA] || dialogs.smallTalk.female);
            const lineB = pick(dialogs.replies.generic[genderB] || dialogs.replies.generic.male);
  ...
  onUserMessage(channel, nick, text) {
    ...
    for (const botNick of Object.keys(this.state)) {
      ...
      if (pattern.test(text)) {
        ...
        if (bot && bot.online && Math.random() < config.reactions.onDirectMention) {
          ...
          let reply = '';
          if (lower.includes('?')) {
            reply = pick(dialogs.replies.question[genderKey] || dialogs.replies.question.female);
          } else if (/\b(salam|aoa|hi|hello|hey|yo|kaisay)\b/.test(lower)) {
            reply = pick(dialogs.greetings[genderKey] || dialogs.greetings.female);
          } else {
            reply = pick(dialogs.replies.targeted[genderKey] || dialogs.replies.targeted.female);
          }
    ...
    if (/\b(salam|aoa|assalam|hi|hello|hey|yo|sup|greetings|wb)\b/.test(lower) && text.length < 35) {
      if (Math.random() < config.reactions.onGreeting) {
        ...
        if (b) {
          ...
          this.sayWithTyping(chan, b.nick, this.humanize(pick(pool), nick, b.nick), rand(400, 1400));
            ...
            setTimeout(() => {
              ...
              if (b2) {
                const b2Greeting = b2.gender === 'f'
                  ? pick(['walekum assalam ' + nick + '! ^^', 'salam jani :)', 'welcome! <3'])
                  : pick(['walekum assalam ' + nick + ' bhai', 'salam bro :)', 'welcome jani!']);
    ...
    for (const [kw, genderReplies] of Object.entries(dialogs.replies.keyword)) {
      if (lower.includes(kw) && Math.random() < config.reactions.onKeyword) {
        ...
        if (b) {
          ...
          this.sayWithTyping(chan, b.nick, this.humanize(pick(replies), nick, b.nick), rand(600, 1800));
          return;
        }
      }
    }
    ...
    if (text.includes('?') && Math.random() < config.reactions.onQuestion) {
      ...
      if (b) {
        ...
        this.sayWithTyping(chan, b.nick, this.humanize(pick(replies), nick, b.nick), rand(800, 2000));
        return;
      }
    }
    ...
    if (Math.random() < config.reactions.onGeneralChat) {
      ...
      if (b) {
        ...
        this.sayWithTyping(chan, b.nick, this.humanize(pick(replies), nick, b.nick), rand(1000, 2200));
      }
    }
  }
